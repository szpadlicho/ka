      ...
      <div itemprop="articleBody">
            ...
            {if (Configuration::get('PG_ID1'))}
                <div class="sdsarticle-des"> 
                   {if $post.post_format != 'quote'}
                        ...
                        {if $premium == '1' && ({Group::getCurrent()->id} == {Configuration::get('PG_ID1')} || {Context::getContext()->customer->id} == {Configuration::get('PG_ID3')})}
                        
                            {*<p>post is for premium, user have premium</p>*}
                            {$blogcontent|escape:'quotes':'UTF-8'|replace:"\'":"'"}
                            
                        {elseif $premium == '1' && {Group::getCurrent()->id} != {Configuration::get('PG_ID1')}}
                        
                            {*<p>post is for premium, user not have premium</p>*}
                            {$post.short_description}
                            <br />
                            <div class="premium-info-container">
                                <p class="premium-info">Treść tylko dla użytkowników <a href="{$base_dir}{Configuration::get('PG_LI1')}">PREMIUM</a></p>
                            </div>
                            
                        {elseif $premium == '0' && {Group::getCurrent()->id} == {Configuration::get('PG_ID1')}}
                        
                            {*<p>post is not premium, user have premium</p>*}
                            {$blogcontent|escape:'quotes':'UTF-8'|replace:"\'":"'"}
                            
                        {elseif $premium == '0' && {Group::getCurrent()->id} != {Configuration::get('PG_ID1')}}
                        
                            {*<p>post is not premium, user not have premium</p>*}
                            {$blogcontent|escape:'quotes':'UTF-8'|replace:"\'":"'"}
        ...
        <div class="sdsarticleBottom">
            <span>
                ...
                </span> &nbsp;<i class="icon icon-comments"></i>&nbsp; {if $countcomment != ''}{$countcomment|escape:'htmlall':'UTF-8'}{else}{l s='0' mod='smartblog'}{/if}{l s=' Comments' mod='smartblog'}
            </span>
            <a title="" style="display:none" itemprop="url" href="#"></a>
        </div>
...
{if ($enableguestcomment==0) && isset($is_looged) && $is_looged>0}
	...
	<div class="right">
        ...
        <div class="submit">
           <button type="submit" name="addComment" id="submitComment" class="button btn btn-default button-medium" >{l s='Submit' mod='smartblog'}</button>
		</div>
