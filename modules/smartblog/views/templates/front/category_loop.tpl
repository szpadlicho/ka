            ...
            <span>
                {if $smartshowauthor ==1}{l s='Posted by' mod='smartblog'} 
                    <span itemprop="author">&nbsp;<i class="icon icon-user"></i>&nbsp; 
                        {if $smartshowauthorstyle != 0}{$post.firstname|escape:'htmlall':'UTF-8'}
                            {$post.lastname|escape:'htmlall':'UTF-8'}
                        {else}
                ...
                {if $smartshowviewed ==1}
                    &nbsp; <i class="icon icon-eye-open"></i>{l s=' views' mod='smartblog'}
                    ({$post.viewed|intval})
                {/if}
            </span>
            {if $post.post_format != 'quote' && $post.post_format != 'link'}
                <div class="sdsreadMore">  
                    <a title="{$post.meta_title|escape:'htmlall':'UTF-8'}" href="{$smartbloglink->getSmartBlogPostLink($post.id_post,$post.link_rewrite)|escape:'htmlall':'UTF-8'}" class="r_more btn btn-default button button-small">
                        <span>{l s='Read more' mod='smartblog'}<i class="icon-chevron-right right"></i></span>
                    </a>
                </div>
            {/if}  
        </div>
