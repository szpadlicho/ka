...
class smartblog extends Module
{
    ...
    public function install()
    {
        ...
        Configuration::updateGlobalValue('smartdataformat', 'd/m/Y H:i:s');
        Configuration::updateGlobalValue('smartblogurlpattern', 2);

        Configuration::updateGlobalValue('smartblogmetatitle', 'Blog - Dziadkiewicz Bogusław - Kancelaria radcy prawnego');
        Configuration::updateGlobalValue('smartblogmetakeyword', 'blog,kancelaria,radcowska,boguslaw,dziadkiewicz');
        Configuration::updateGlobalValue('smartblogmetadescrip', 'DZIADKIEWICZ BOGUSŁAW - KANCELARIA RADCY PRAWNEGO BLOG');
        ...
        if (!parent::install() || !$this->registerHook('displayHeader') || 
			...
			!$this->registerHook('BlogCategory') ||
			!$this->registerHook('BlogSearch') 
        )
            return false;
        $sql = array();
...
//    }
    ...
    public function SampleDataInstall()
    {
        ...
        foreach ($languages as $language) {
            $damisql2 = "INSERT INTO " . _DB_PREFIX_ . "smart_blog_category_lang (id_smart_blog_category,name,meta_title,id_lang,link_rewrite) VALUES (1,'Główna','Główna','" . (int) $language['id_lang'] . "','glowna');";
        ...
        for ($i = 1; $i <= 4; $i++) {
            if ($i == 1):
                $title = 'Tittle for home';
                $slug = 'tittle-for-home';
            ...
            elseif ($i == 2):
                $title = 'Tittle for long';
                $slug = 'tittle-for-long';
            ...
            elseif ($i == 3):
                $title = 'Tittle for prawne';
                $slug = 'tittle-for-prawne';
                $des = 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industrys standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged.';
            elseif ($i == 4):
                $title = 'Tittle for fizyczne';
                $slug = 'tittle-for-fizyczne';
                $des = 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industrys standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged.';
            elseif ($i == 5):
    ...
    /**/
    public function smartblogcategoriesHookBlogCategory($params)
    {


        if (!$this->isCached('plugins/smartblogcategories2.tpl')) {
            $view_data = array();
            $id_lang = $this->context->language->id;
            $BlogCategory = new BlogCategory();
            $categories = $BlogCategory->getCategory(1, $id_lang);
        ...
        return $this->display(__FILE__, 'views/templates/front/plugins/smartblogcategories2.tpl');
    }
    public function hookBlogCategory($params)
    {
        return $this->smartblogcategoriesHookBlogCategory($params);
    }
    public function hookBlogSearch($params)
    {
        return $this->smartblogcategoriesHookBlogCategory($params);
    }
    /**/
