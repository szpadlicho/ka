    ...
    public function __call($function, $args)
    {
        ...
        if ($hook == 'hook') {
            ...
            if(($hook_name=='displayFullWidthTop' || $hook_name=='displayFullWidthTop2' || $hook_name=='displayFullWidthBottom') && Dispatcher::getInstance()->getController()!='index')
              return false;
            if($hook_name=='displayComingSoon')
            {
                $this->context->controller->addJquery();
                $this->hookdisplayHeader();
                $css_js = '';
                foreach ($this->context->controller->css_files as $css_uri => $media) {
                    $css_js .= '<link rel="stylesheet" href="'.$css_uri.'" type="text/css" media="'.$media.'" />';
                }
                foreach ($this->context->controller->js_files as $js_uri) {
                    $css_js .= '<script type="text/javascript" src="'.$js_uri.'"></script>';
                }
                return $css_js.$this->generateSlider($hook_name);
            }
            else
